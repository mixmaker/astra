...
class LatestWallsPage extends StatelessWidget {
  LatestWallsPage({super.key});

  final Controller controller = Get.find();

  Future<void> _onRefresh() async {
    controller.updateLatest(ListOfWalls(data: <Wall>[]));
    var popularData = await latestWalls(
        isFamilyFilter: controller.isFamilyFilter.value.toString());
    controller.updateLatest(popularData ?? <Wall>[]);
  }
                                ...
                                constraints: BoxConstraints(
                                    minHeight:
                                        MediaQuery.of(context).size.height -
                                            200),
                                child: controller.latestWalls().data.isEmpty
                                    ? Padding(
